...
class SaveDepartment(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        try:
            if id > 0:
                dept = models.Departments.objects.exclude(
...
class SaveVisitor(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('name', 'department', 'gender',
                  'contact', 'email', 'address', 'reason', )

    def clean_department(self):
        did = self.cleaned_data['department'] if self.cleaned_data['department'].isnumeric(
        ) else 0
        try:
            if int(did) > 0:
